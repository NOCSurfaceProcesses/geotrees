...
from geotrees.octtree import OctTree, PolarsOctTree
...
from geotrees.utils import DateWarning, FailedInsertWarning
...
def test_intersection():
    ...
    test_rects: list[Rectangle] = [
        Rectangle(1, 19, 1, 9, d - timedelta(days=1), d + timedelta(days=1)),
        Rectangle(
            20.5,
            29.5,
            -1,
...
def test_divides():
    ...
    otree2 = PolarsOctTree(boundary)
    otree2.divide()
    res2 = [
        otree2.northwestback.boundary,
        otree2.northeastback.boundary,
        otree2.southwestback.boundary,
        otree2.southeastback.boundary,
        otree2.northwestfwd.boundary,
        otree2.northeastfwd.boundary,
        otree2.southwestfwd.boundary,
        otree2.southeastfwd.boundary,
    ]
...
def test_insert():
    ...
    points: list[Record] = [
        ...
        Record(12, 1, d + timedelta(hours=3), "southeastfwd1"),
        Record(3, 7, d - timedelta(days=3), "northwestback2"),
        Record(13, 2, d + timedelta(hours=17), "southeastfwd2"),
        Record(13, 6, d - timedelta(days=1), "northeastback1"),
...
def test_insert_pl():
    ...
    boundary = Rectangle(0, 20, 0, 8, start, end)
    otree = PolarsOctTree(boundary, capacity=3)
    points: list[Record] = [
        Record(10, 4, d, "northwestback1"),
        Record(12, 1, d + timedelta(hours=3), "southeastfwd1"),
        Record(3, 7, d - timedelta(days=3), "northwestback2"),
    ...
    frame = pl.from_dicts([p.__dict__ for p in points])
    with pytest.warns(FailedInsertWarning):
        otree.insert(frame)
    assert otree.len() == frame.height - 1
    assert otree.divided
    assert otree.data is None

    expected = [
        ["northwestback2", "northwestback1"],
        ["northeastback2", "northeastback1"],
        [],
        [],
        [],
        ["northeastfwd"],
        ["southeastfwd2", "southeastfwd1"],
        [],
    ]
    res = [
        # otree.points,
        otree.northwestback.uids,
        otree.northeastback.uids,
        otree.southwestback.uids,
        otree.southeastback.uids,
        otree.northwestfwd.uids,
        otree.northeastfwd.uids,
        otree.southeastfwd.uids,
        otree.southwestfwd.uids,
    ]
    for ex, r in zip(expected, res):
        ex.sort()
        r.sort()
    ...
    boundary2 = Rectangle(170, -170, 0, 8, start, end)
    otree2 = PolarsOctTree(boundary2, capacity=3)
    otree2.insert(frame2)
    assert otree2.divided
    assert otree2.len() == frame.height - 1
    res2 = [
        # otree.points,
        otree2.northwestback.uids,
        otree2.northeastback.uids,
        otree2.southwestback.uids,
        otree2.southeastback.uids,
        otree2.northwestfwd.uids,
        otree2.northeastfwd.uids,
        otree2.southeastfwd.uids,
        otree2.southwestfwd.uids,
    ]
    print(res2)
    for ex, r in zip(expected, res2):
        ex.sort()
        r.sort()
        assert ex == r
...
def test_remove_pl():
    ...
    frame = pl.from_dicts([p.__dict__ for p in points])
    to_remove = frame.row(4, named=True)
    with pytest.warns(FailedInsertWarning):
        otree.insert(frame)
    lon = to_remove["lon"]
    lat = to_remove["lat"]
    dt = to_remove["datetime"]

    # TEST: query works before remove
    q_res = otree.nearby_points(
        lat=lat, lon=lon, datetime=dt, dist=0.1, t_dist=timedelta(minutes=5)
    )
    assert len(q_res) == 1
...
def test_query():
    ...
    res = otree.nearby_points(test_point, dist=200, t_dist=timedelta(hours=5))
...
def test_query_pl():
    ...
    lon, lat, dt = 11, 6, d + timedelta(hours=4)
    test_rect = Rectangle(10, 11.5, 6.5, 7.5, d, d + timedelta(hours=6))
    expected_uid = pl.Series(["northeastfwd"])

    res = otree.nearby_points(
        lat=lat, lon=lon, datetime=dt, dist=200, t_dist=timedelta(hours=5)
    )
    assert res.height == 1
